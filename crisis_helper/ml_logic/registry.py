...
def save_encoder(encoder: sklearn.preprocessing._label.LabelEncoder = None) -> None:

    """
    Saves encoder locally in a pickle file
    """
    ...
    if encoder is not None:
        models_path = os.path.join(LOCAL_REGISTRY_PATH, "encoder", timestamp + ".pickle")
        with open(models_path, 'wb') as file:
            pickle.dump(encoder, file)

        print("✅ encoder saved locally")
        return None
...
def load_encoder() -> sklearn.preprocessing._label.LabelEncoder:
    #load_encoder(stage="Production") -> sklearn.feature_extraction.text.TfidfVectorizer:
    """
    Return a saved model:
    - locally (latest one in alphabetical order)
    - or from GCS (most recent one) if MODEL_TARGET=='gcs'  --> for unit 02 only
    ...
    # Get the latest model version name by the timestamp on disk
    local_encoder_directory = os.path.join(LOCAL_REGISTRY_PATH, "encoder")
    local_encoder_files = glob.glob(f"{local_encoder_directory}/*.pickle")

    if not local_encoder_files:
        return None

    most_recent_encoder_file_on_disk = sorted(local_encoder_files)[-1]

   # Open saved encoder
    with open(most_recent_encoder_file_on_disk,'rb') as f:
        latest_encoder = pickle.load(f)

    print("✅ encoder loaded from local disk")
